...
function checkInput(pattern, enteredData, className) {
  const resultLineValue = document.querySelector(className);

  if (pattern.test(enteredData.value)) {
    if (resultLineValue.textContent.includes(enteredData.value)) {
    } else {
      resultLineValue.textContent += enteredData.value;
    }
